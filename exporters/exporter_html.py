...
.actions,.result-actions{display:flex;gap:8px}.primary{border-color:var(--blue);color:white;background:var(--blue)}.result-bar{margin-bottom:10px}.result-actions{align-items:center;flex-wrap:wrap}.count{color:var(--muted);font-size:13px}.mark-all{color:#8a5200;border-color:#d4a45e}.clear-all{color:#8d2e38}
...
.title{display:flex;justify-content:space-between;gap:14px}.title h3{margin:0;font-size:16px}.mail-actions{display:flex;align-items:center;gap:12px;flex:none}.important{display:flex;align-items:center;gap:5px;color:#9a5a00;font-size:12px;font-weight:bold;cursor:pointer}.important input{width:auto;accent-color:#d98200}.number{color:var(--muted);font-size:12px}
...
</aside><section><div class="result-bar"><strong>検索結果</strong><div class="result-actions"><span id="result-count" class="count"></span><button id="mark-visible" class="mark-all" type="button">表示中をすべて重要</button><button id="unmark-visible" class="clear-all" type="button">表示中の重要を解除</button></div></div><div id="mail-list" class="mail-list"></div></section></main>
...
const storageKey="mail-visualizer-important-v1";
const mailKey=mail=>mail.message_id||`${mail.no}|${mail.date}|${mail.subject}`;
let savedMarks={};try{savedMarks=JSON.parse(localStorage.getItem(storageKey)||"{}")}catch(error){savedMarks={}}
mails.forEach(mail=>mail.important=Boolean(savedMarks[mailKey(mail)]));
function saveMark(mail){savedMarks[mailKey(mail)]=mail.important;try{localStorage.setItem(storageKey,JSON.stringify(savedMarks))}catch(error){/* 保存不可の場合も現在の画面内では利用できる */}}
const fields=[["important","重要チェック"],["no","管理No."],["date","原文日時"],["date_jst","日本時間（JST）"],["sender","差出人"],["to","宛先"],["subject","件名"],["body","本文"],["message_id","Message-ID"]];
...
const fieldValue=(mail,field)=>field==="important"?(mail.important?"重要":"未チェック"):mail[field];
function matches(mail,c){const v=norm(fieldValue(mail,c.field)),q=norm(c.query);if(!q)return true;if(c.operator==="not_contains")return !v.includes(q);if(c.operator==="equals")return v===q;if(c.operator==="starts")return v.startsWith(q);if(c.operator==="ends")return v.endsWith(q);return v.includes(q)}
function compare(a,b,field){if(field==="important")return Number(a==="重要")-Number(b==="重要");if(field==="no")return Number(a||0)-Number(b||0);if(field==="date"||field==="date_jst"){const left=Date.parse(a),right=Date.parse(b);if(!Number.isNaN(left)&&!Number.isNaN(right))return left-right}return norm(a).localeCompare(norm(b),"ja",{numeric:true,sensitivity:"base"})}
...
function render(items){visibleMails=items;list.replaceChildren();document.getElementById("result-count").textContent=`${items.length} / ${mails.length}件`;if(!items.length){list.append(element("div","条件に一致するメールはありません。","empty"));return}
 for(const mail of items){const details=document.createElement("details");details.className="mail";const summary=document.createElement("summary"),title=document.createElement("div");title.className="title";title.append(element("h3",mail.subject||`件名なし ${mail.no}`));const actions=document.createElement("div");actions.className="mail-actions";const mark=document.createElement("label");mark.className="important";const checkbox=document.createElement("input");checkbox.type="checkbox";checkbox.checked=mail.important;checkbox.onclick=event=>event.stopPropagation();checkbox.onchange=()=>{mail.important=checkbox.checked;saveMark(mail);applyConditions()};mark.onclick=event=>event.stopPropagation();mark.append(checkbox,document.createTextNode("重要"));actions.append(mark,element("span",`No. ${mail.no}`,"number"));title.append(actions);const meta=document.createElement("div");meta.className="meta";meta.append(element("span",`From: ${mail.sender||"―"}`),element("span",`To: ${mail.to||"―"}`),element("span",mail.date_jst||mail.date||"日時なし"));summary.append(title,meta);const body=document.createElement("div");body.className="body";const dl=document.createElement("dl");[["差出人",mail.sender],["宛先",mail.to],["原文日時",mail.date],["日本時間（JST）",mail.date_jst],["Message-ID",mail.message_id]].forEach(([k,v])=>dl.append(element("dt",k),element("dd",v||"―")));body.append(dl,element("pre",mail.body||"本文なし"));details.append(summary,body);list.append(details)}}
function applyConditions(){const filterConditions=[...document.querySelectorAll(".filter-condition")].map(r=>({field:r.querySelector(".field").value,operator:r.querySelector(".operator").value,query:r.querySelector(".query").value}));const sortConditions=[...document.querySelectorAll(".sort-condition")].map(r=>({field:r.querySelector(".field").value,direction:r.querySelector(".direction").value}));const result=mails.filter(mail=>filterConditions.every(c=>matches(mail,c)));result.sort((a,b)=>{for(const c of sortConditions){const compared=compare(fieldValue(a,c.field),fieldValue(b,c.field),c.field);if(compared)return c.direction==="desc"?-compared:compared}return Number(a.no)-Number(b.no)});render(result)}
...
function setVisibleMarks(important){for(const mail of visibleMails){mail.important=important;savedMarks[mailKey(mail)]=important}try{localStorage.setItem(storageKey,JSON.stringify(savedMarks))}catch(error){}applyConditions()}
document.getElementById("mark-visible").onclick=()=>setVisibleMarks(true);
document.getElementById("unmark-visible").onclick=()=>{if(visibleMails.length&&confirm(`表示中の${visibleMails.length}件から重要チェックを外しますか？`))setVisibleMarks(false)};
